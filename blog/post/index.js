...
function BlogPostSidebar(context, { toc }) {
  ...
  return html` <nav class="blog-toc">${ReferenceToc(context)}</nav> `;
}
...
function RenderBlogContent(_context, { doc }) {
  return html` ${doc.body.map((section) => Section(section, null))} `;
}
...
export function BlogPost(context) {
  ...
  if (!blogMeta || !doc) {
    return PageLayout(
      context,
      html`<p>${context.l10n("blog_post_not_found")`Blog post not found`}</p>`,
    );
  }
  const { toc } = doc;
  ...
  return PageLayout(context, BlogContainer(context, postContent));
}
