...
/**
 * One row per group; remaining browsers form the last row.
 */
const BROWSER_ROWS = [BASELINE_BROWSERS, WEBVIEW_BROWSERS];

/**
 * Previews browser choices on the parent table; persists them on Save.
 */
export class MDNCompatTableSettings extends L10nMixin(LitElement) {
  ...
  async _open() {
    ...
    // Render before opening to avoid pill animations.
  ...
  _preview(visibility) {
    this.dispatchEvent(
      new CustomEvent("mdn-compat-table-settings-preview", {
        detail: visibility,
        bubbles: true,
        composed: true,
      }),
  ...
  _save() {
    // Keep following future defaults when no customization remains.
  ...
  _renderBrowser(browser, selected) {
    ...
    >
      <span
        class=${`icon icon-browser icon-${browserToIconName(browser)}`}
      ></span>
  ...
  render() {
    return html`
      <mdn-button variant="plain" .icon=${settingsIcon} @click=${this._open}
        >${this.l10n("compat-settings-open")`Settings`}</mdn-button
      >
      <mdn-modal
        closedby="closerequest"
        modal-title=${this.l10n("compat-settings-title")`Browser compatibility settings`}
        @close=${this._onClose}
      >
        <p class="intro">
          ${this.l10n(
