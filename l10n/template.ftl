...
playground-run-warning-line-one = This is a user-shared playground, not official MDN content.
playground-run-warning-line-two = Always inspect the code first, and run it at your own risk.
playground-run-warning-line-three = Running malicious code can expose you to serious risks, including theft of data, IP address, browser fingerprint, current session, etc.
playground-run-warning-line-four = Never enter passwords, payment details, address, or any sensitive information.
