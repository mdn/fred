article-footer-last-modified = 此页面最后更新于 <time data-l10n-name="date">{ $date }</time>，更新者为 <a data-l10n-name="contributors">MDN 贡献者</a>。
article-footer-source-title = 文件夹：{ $folder }（在新标签页中打开）
baseline-asterisk = 此功能的部分特性在各浏览器中的支持情况不一致。
baseline-high-extra = 此功能已较为成熟，可在大多数设备和浏览器版本中正常使用。自 { $date } 起，已在主流浏览器中得到支持。
baseline-low-extra = 自 { $date } 起，此功能可在最新设备和浏览器版本中运行。此功能在较旧的设备或浏览器中可能无法运行。
baseline-not-extra = 此功能不属于基线，因为它无法在一些最广泛使用的浏览器中运行。
baseline-supported-in = 在 { $browsers } 中受支持
baseline-unsupported-in = 在 { $browsers } 中未被广泛支持
baseline-supported-and-unsupported-in = 在 { $supported } 中受支持，但在 { $unsupported } 中未被广泛支持
...
not-found-fallback-english = <strong data-l10n-name="strong">好消息：</strong>你请求的页面有 <em data-l10n-name="em">英文版</em>。
not-found-fallback-search = 你请求的页面不存在，但你可以尝试站内搜索：
not-found-back = 返回首页
footer-mofo = 访问 <a data-l10n-name="moco">Mozilla Corporation</a> 的非营利母机构 <a data-l10n-name="mofo">Mozilla Foundation</a>。
footer-copyright = 本内容的部分版权归 mozilla.org 贡献者所有，©1998–{ $year }。内容依据 <a data-l10n-name="cc">Creative Commons 许可协议</a>提供。
...
compat-support-removed = 在 { $version } 及更高版本中已移除
...
compat-legend-disabled = 用户必须显式启用此功能。
...
article-footer-this-will-take-you-to-github-to = 这将带你前往 GitHub 提交新问题。
...
baseline-indicator-newly-available = 新近可用
...
collection-save-button-saved-articles = 已保存文章
...
color-theme-dark = 深色
color-theme-switch-color-theme = 切换颜色主题
...
compat-legend = 图例
compat-legend-tip = 提示：你可以点击/轻触单元格以获取更多信息。
...
content-feedback-question = 此页面对你有帮助吗？
content-feedback-yes = 是
content-feedback-no = 否
content-feedback-reason = 为什么此页面对你没有帮助？
content-feedback-submit = 提交
content-feedback-thanks = 感谢你的反馈！
contributor-spotlight-want-to-be-part-of-the-journey = 想成为这段旅程的一部分吗？
contributor-spotlight-our-constant-quest-for-innovatio = 我们对创新的不懈追求始于此，也始于你。MDN 的每一部分（文档、演示以及网站本身）都源于我们了不起的开放开发者社区。欢迎加入我们！
...
copy-button-copy = 复制
footer-mdn-on-github = GitHub 上的 MDN
footer-mdn-on-bluesky = MDN 在 Bluesky 上
footer-mdn-on-x = MDN 在 X 上
footer-mdn-on-mastodon = MDN 在 Mastodon 上
...
interactive-example-the-current-value-is-not-support = 当前值不受你的浏览器支持。
...
observatory-tests-and-scores-description = 描述
observatory-tests-and-scores-modifier = 修正项
...
playground-do-you-really-want-to-clear-ever = 你确定要清空所有内容吗？
playground-do-you-really-want-to-revert-you = 你确定要撤销你的更改吗？
...
playground-share-your-code-via-permalink = 通过永久链接分享你的代码
...
playground-can-you-please-share-some-detail = 你能否提供一些关于此内容存在问题的详细信息：
...
scrim-inline-load-scrim-and-open-dialog = 加载 scrim 并打开对话框。
...
site-search-suggestions-text = 你是否在找……
site-search-searching = 搜索中…
site-search-language = 语言
site-search-both = 两者
specifications-list-this-feature-does-not-appear-to = 此功能似乎未在任何规范中定义。
