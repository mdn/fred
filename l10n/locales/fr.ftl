...
playground-run-warning-line-one = Il s'agit d'un espace de jeu partagé par les utilisateurs, et non d'un contenu officiel de MDN.
playground-run-warning-line-two = Toujours inspecter le code au préalable et l'exécuter à vos propres risques.
playground-run-warning-line-three = L'exécution de code malveillant peut vous exposer à des risques graves, notamment le vol de données, d'adresse IP, d'empreinte digitale du navigateur, de session en cours, etc.
playground-run-warning-line-four = Ne saisissez jamais vos mots de passe, vos informations de paiement, votre adresse ou toute autre information sensible.
